...
dependencies needed for running omnigraph (`[tool.poetry.dependencies]` with the
...
developing omnigraph using test-driven development. Please make sure that you
...
If you are brand new to omnigraph or open-source development, we recommend
...
omnigraph is hosted on GitHub, and to contribute, you will need to sign up for a
...
### Create a fork of omnigraph

You will need your own fork of omnigraphin order to eventually open a Pull
Request. Go to the omnigraph project page and hit the Fork button. Please
...
```bash
git clone https://github.com/your-user-name/omnigraph.git
cd omnigraph
git remote add upstream https://github.com/omnigraph/omnigraph.git
git fetch upstream
```

This creates the directory `omnigraph` and connects your repository to the
upstream (main project) *omnigraph* repository. They have the same name, but
your local repository and fork are separate from the upstream repository.

### Creating a feature branch

Your local `main` branch should always reflect the current state of omnigraph
repository. First ensure it's up-to-date with the main omnigraph repository.
...
to create a branch called `my-new-feature-for-omnigraph`

```bash
git checkout -b my-new-feature-for-omnigraph
```

This changes your working branch from `main` to the
`my-new-feature-for-omnigraph` branch. Keep any changes in this branch specific
to one bug or feature so it is clear what the branch brings to *omnigraph*. You
...
When making changes, follow these omnigraph-specific guidelines:
...
Now your code is on GitHub, but it is not yet a part of the omnigraph project.
...
1. Write a descriptive title that includes prefixes. omnigraph uses a convention for title prefixes, most commonly, `feat:` for features, `fix:` for bug fixes, and `refactor:` for refactoring.
...
branch with changes in the omnigraph `main` branch, run:

```shell

    git checkout my-new-feature-for-omnigraph
    git fetch upstream
...
```shell
    git push origin my-new-feature-for-omnigraph
```
